package cn.com.ecrf.trq.model;

import java.util.List;

public class PersonAllergicHistoryCase {
   //吸烟史
	private String xysyn;
	//饮酒史
	private String yjsyn;
	//食物过敏史
	private String swgmsyn;
	//疑似过敏物
	private String ysgmw;
	//食物过敏表现
	private String swgmbx;
	//药物过敏史
	private String ywgmsyn;
	//抗生素类过敏物
	private String ksslgmw;
	//抗生素类过敏表现
	private String ksslgmbx;
	//中药注射剂过敏物
	private String zyzsjgmw;
	//中药注射剂过敏表现
	private String zyzsjgmbx;
	//其他药物过敏物
	private String qtywgmw;
	//其他药物过敏表现
	private String qtywgmbx;
	//其他物质
	private String qtwzyn;
	//其他药物过敏物
	private String qtwzgmw;
	//其他药物过敏表现
	private String qtwzgmbx;
	//观察表编号
	private String blbh;
	
	public String getXysyn() {
		return xysyn;
	}
	public void setXysyn(String xysyn) {
		this.xysyn = xysyn;
	}
	public String getYjsyn() {
		return yjsyn;
	}
	public void setYjsyn(String yjsyn) {
		this.yjsyn = yjsyn;
	}
	public String getSwgmsyn() {
		return swgmsyn;
	}
	public void setSwgmsyn(String swgmsyn) {
		this.swgmsyn = swgmsyn;
	}
	public String getYsgmw() {
		return ysgmw;
	}
	public void setYsgmw(String ysgmw) {
		this.ysgmw = ysgmw;
	}
	public String getSwgmbx() {
		return swgmbx;
	}
	public void setSwgmbx(String swgmbx) {
		this.swgmbx = swgmbx;
	}
	public String getYwgmsyn() {
		return ywgmsyn;
	}
	public void setYwgmsyn(String ywgmsyn) {
		this.ywgmsyn = ywgmsyn;
	}
	public String getKsslgmw() {
		return ksslgmw;
	}
	public void setKsslgmw(String ksslgmw) {
		this.ksslgmw = ksslgmw;
	}
	public String getKsslgmbx() {
		return ksslgmbx;
	}
	public void setKsslgmbx(String ksslgmbx) {
		this.ksslgmbx = ksslgmbx;
	}
	public String getZyzsjgmw() {
		return zyzsjgmw;
	}
	public void setZyzsjgmw(String zyzsjgmw) {
		this.zyzsjgmw = zyzsjgmw;
	}
	public String getZyzsjgmbx() {
		return zyzsjgmbx;
	}
	public void setZyzsjgmbx(String zyzsjgmbx) {
		this.zyzsjgmbx = zyzsjgmbx;
	}
	public String getQtywgmw() {
		return qtywgmw;
	}
	public void setQtywgmw(String qtywgmw) {
		this.qtywgmw = qtywgmw;
	}
	public String getQtywgmbx() {
		return qtywgmbx;
	}
	public void setQtywgmbx(String qtywgmbx) {
		this.qtywgmbx = qtywgmbx;
	}
	public String getQtwzyn() {
		return qtwzyn;
	}
	public void setQtwzyn(String qtwzyn) {
		this.qtwzyn = qtwzyn;
	}
	public String getQtwzgmw() {
		return qtwzgmw;
	}
	public void setQtwzgmw(String qtwzgmw) {
		this.qtwzgmw = qtwzgmw;
	}
	public String getQtwzgmbx() {
		return qtwzgmbx;
	}
	public void setQtwzgmbx(String qtwzgmbx) {
		this.qtwzgmbx = qtwzgmbx;
	}
	public String getBlbh() {
		return blbh;
	}
	public void setBlbh(String blbh) {
		this.blbh = blbh;
	}
	
	
	
}
